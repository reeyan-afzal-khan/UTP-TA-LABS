...
\medskip
\textbf{The spreadsheet was not included with the course materials.} Rather than leave the lab uncompletable, \texttt{Lab06/sop\_planner.py} reproduces the same calculation with every formula written out:

\begin{lstlisting}[style=erpterminal]
py sop_planner.py
py sop_planner.py --safety-stock 500 --unit pairs
\end{lstlisting}

Build the spreadsheet as the tutorial requires --- that is the deliverable. Use the script to \emph{check} it. If your sheet and the script disagree, one of you has a formula wrong, and finding out which is worth more than either answer.
\end{labmode}
