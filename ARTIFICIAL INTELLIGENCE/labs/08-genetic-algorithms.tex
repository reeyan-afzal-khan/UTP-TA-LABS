...
\textbf{Goal.} Implement a GA in plain NumPy, verify it on a toy objective, run it over ten seeds on the medical-cost data, and explain when an evolutionary optimiser is or is not justified.\\
\textbf{Data.} \texttt{all\_datasets/lab08-insurance.csv} (1,338 policies).\\
\textbf{Source files.} \texttt{all\_experiments/lab08\_genetic\_algorithm\_regression\_worked.ipynb}.\\
\end{labproject}

\begin{labmode}
The GA is written twice on purpose: a short scalar version in Question~1 whose answer is known ($x=3$), and the regression version in Question~3 with the same loop. If the first one does not converge, fix it before going on --- a bug is much easier to see when the right answer is a single number.
\end{labmode}
