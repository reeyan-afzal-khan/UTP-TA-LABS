...
\textbf{Goal.} Build a leakage-safe benchmark from the single supplied \texttt{Tweets.csv}: a majority-class baseline and a Multinomial Naive Bayes model tuned on validation data and evaluated once on an untouched test set.\\
\textbf{Data.} \texttt{all\_datasets/lab07-Tweets.csv} (14,640 tweets about six US airlines).\\
\textbf{Source files.} \texttt{all\_experiments/lab07\_airline\_sentiment\_worked.ipynb}.\\
\end{labproject}

\begin{labmode}
The purpose is model evaluation, not ranking airlines: subgroup scores reflect sample size, topic mix and class balance as much as service quality. The test subset is created in Question~2 and read exactly once, in Question~6; every decision before that is made on training or validation rows.
\end{labmode}
