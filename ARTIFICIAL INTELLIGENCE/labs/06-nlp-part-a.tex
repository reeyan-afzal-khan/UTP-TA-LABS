...
\begin{labmode}
\texttt{pytesseract} is only a wrapper. Live OCR requires the
Tesseract \emph{engine}; if it is unavailable, the notebook clearly reports that it is
using the included validated predictions instead.
...
Submit the executed notebook, the reported OCR mode (and Tesseract version for a live run), and
\texttt{results/lab06/}. State which OCR stage improved, which failure remained, and why
the tested pipeline is not suitable for autonomous clinical transcription.
\end{labdeliverable}
