...
\textbf{Objective.} Build a Mamdani controller and a compact Sugeno ANFIS, then compare ANFIS with linear regression on held-out data.\\
\textbf{Data.} \texttt{all\_datasets/lab04-AirQuality.csv} (UCI Air Quality, hourly sensor readings).\\
\textbf{Notebook.} \texttt{all\_experiments/lab04\_fuzzy\_anfis\_worked.ipynb}; inference and ANFIS training are implemented with NumPy, SciPy, and scikit-learn.\\
\end{labproject}
...
Submit the executed notebook and \texttt{results/lab04/}. Briefly justify the selected
ANFIS size, explain why the test set cannot be used for model selection, and state one
limitation of the random split for time-ordered sensor data.
\end{labdeliverable}
