...
\begin{tikzpicture}[
  q/.style={draw=MLNavy,fill=MLNavy,text=white,rounded corners=2mm,
            text width=3.3cm,minimum height=0.9cm,align=center,font=\bfseries\small},
  ...
  leaf/.style={draw=MLTeal,fill=MLPaleTeal,rounded corners=2mm,
               text width=3.0cm,minimum height=1.18cm,align=center,font=\scriptsize\color{MLNavy}},
  arr/.style={-{Stealth[length=2mm]},thick,draw=MLMuted}
]
\node[q] (start) at (0,3.8) {What feedback is available?};

\node[branch] (full) at (-5.4,1.9) {A target for each example};
\node[branch] (few) at (-1.8,1.9) {Only some targets};
\node[branch] (none) at (1.8,1.9) {No external target};
