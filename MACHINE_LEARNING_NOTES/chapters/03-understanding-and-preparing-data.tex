...
\chapterguide%
  ...
  {assess and clean a dataset, distinguish deterministic corrections from learned preprocessing, choose a valid split, handle missing and categorical values, prevent leakage, and build a reproducible pipeline.}

Now that the environment and basic library vocabulary are in place, the next job is to make the data trustworthy. A powerful algorithm cannot rescue a badly defined label, incorrect measurements, or an unfair train-test split.
...
\begin{mlsteps}
  ...
  \item \term{When is the label known?} A feature available only after the outcome occurs creates leakage.
...
\midrule
Before the split & Check required columns, parse known date formats, standardize documented units, normalize harmless whitespace, verify keys, and reject impossible values using domain rules. & These rules do not estimate anything from the label or from the distribution of the full dataset. \\
After the split & Learn medians, means, scales, frequent-category cutoffs, outlier thresholds, label-informed mean encodings, and any later feature-selection or resampling rules. & These choices depend on observed data and must be fitted on training rows only. \\
...
\begin{keyidea}
A correction may happen before the split when it follows a fixed, label-blind rule that would be applied identically to any future row. If a value, cutoff, category grouping, or decision is estimated from the observed data, learn it from training data only.
\end{keyidea}
...
Label-aware & Label-informed mean encoding, supervised feature selection, a category merge chosen because it separates labels & Fit inside training folds; never compute from the complete labeled table before validation. Supervised selection is treated as a leakage-sensitive model-development step in \chapref{ch:training}. \\
...
\begin{enumerate}
  ...
  \item Why is a prediction-time information boundary necessary when deciding whether a feature is leakage?
\end{enumerate}
\end{quickcheck}

\begin{chapterrecap}
\begin{itemize}
  \item A data quality assessment begins with the meaning of rows, columns, units, timing, and missing populations.
  \item Fixed, label-blind schema corrections can precede the split; quantities learned from observed data belong inside training.
