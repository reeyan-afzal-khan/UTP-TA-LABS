...
\chapterguide%
  ...
  {Fit and interpret logistic regression, extend it to many classes with softmax, explain how discriminant analysis reaches a linear boundary from a different direction, recognize ordered-category targets, and place the perceptron in its historical role.}
...
\begin{firstread}
Core path: logistic regression, cross-entropy, decision thresholds, and softmax regression. Ordinal targets are a useful special case. LDA and the perceptron are comparison models for a second pass rather than prerequisites for later chapters.
\end{firstread}
...
\label{sec:ordinal-targets}

Some targets are categories with a meaningful order: \emph{low $<$ medium $<$ high}, satisfaction ratings, or disease stages. This is an \term{ordinal} target. Ordinary softmax classification ignores the order, while coding the labels as $1,2,3$ and using ordinary regression invents equal numerical gaps that may not exist.
...
There is no single universally appropriate model for ordinal targets. The important first-course skill is recognizing the ordered target structure and choosing an order-aware method or metric when the application requires it. Validation still decides whether that added structure improves the real task.
...
\input{chapters/kaggle-labs/10-logistic}
