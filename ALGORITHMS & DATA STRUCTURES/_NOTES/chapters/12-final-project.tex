\chapter{Final Project: Algorithm Comparison}
\label{ch:final-project}
...
\begin{mltable}
\begin{tabularx}{\linewidth}{@{}>{\bfseries\color{ADSNavy}}p{0.24\linewidth}X X@{}}
\toprule
...
\begin{mltable}
\begin{tabularx}{\linewidth}{@{}>{\bfseries\color{ADSNavy}}p{0.23\linewidth}X@{}}
\toprule
\rowcolor{ADSPaleBlue!92}
...
\begin{mlfigure}
\begin{tikzpicture}[
  box/.style={draw=ADSBlue,fill=ADSPaleBlue,rounded corners=2mm,minimum width=3.1cm,minimum height=0.95cm,align=center,font=\small\bfseries\color{ADSNavy}},
  arr/.style={-{Stealth[length=2mm]},thick,draw=ADSTeal}
]
\node[box] (input) {Input / Generate Data};
\node[box,right=0.7cm of input] (run) {Run Both Algorithms};
\node[box,right=0.7cm of run] (metrics) {Collect Metrics};
\node[box,below=0.8cm of run] (compare) {Side-by-Side Result + Visual};
\draw[arr] (input)--(run);
\draw[arr] (run)--(metrics);
\draw[arr] (metrics.south) |- (compare.east);
...
\begin{mltable}
\begin{tabularx}{\linewidth}{@{}>{\bfseries\color{ADSNavy}}p{0.30\linewidth}X@{}}
\toprule
\rowcolor{ADSPaleBlue!92}
