...
Big-O notation describes an asymptotic upper bound on how resource use grows with input size $n$; it is not an exact operation count. The table and figure use the same eight representative classes, ordered from the slowest to the fastest asymptotic growth.
...
$O(n^3)$ & Polynomial (cubic) & $f(n)=n^3$ & Triple-nested loops; Floyd--Warshall. \\
...
\draw[curve,draw=ADSRed!55!black]
  (start) .. controls (3.28,0.43) and (6.15,1.38) .. (7.06,7.30);
...
\node[directlabel,text=magenta!82!red,anchor=west] at (3.02,7.04)
  {Exponential -- $O(2^n)$};
\node[directlabel,text=ADSRed!55!black,rotate=59,anchor=west,
  fill=white,fill opacity=0.92,text opacity=1] at (5.80,3.08)
  {Polynomial -- $O(n^3),\ldots$};
