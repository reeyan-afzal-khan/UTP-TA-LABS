...
The final learning signal changes more radically. Supervised learning receives labeled examples, unsupervised learning receives no external answer key, and reinforcement learning receives rewards after actions. Reinforcement learning (RL) studies decisions that affect what happens next. Instead of receiving the correct answer for every action, an agent receives rewards and must discover which behavior produces the best long-term outcome.
...
\begin{workedexample}
Suppose $Q(s,a)=2$, reward $r=1$, $\gamma=0.9$, the best next-state Q-value is $4$, and $\alpha=0.5$. The backup value is $1+0.9(4)=4.6$. The temporal-difference error is $4.6-2=2.6$. The update gives
\[
