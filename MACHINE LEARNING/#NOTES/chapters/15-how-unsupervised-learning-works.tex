...
\chapterguide%
  ...
  {explain what changes when no label is supplied, distinguish the main unsupervised tasks, evaluate structure without pretending there is always a ground-truth answer, and follow a safe unsupervised workflow.}

Stage 4A begins by resetting the workflow before any unsupervised model family is chosen. Until now, the main modeling chapters had a label $y$. That label gave training a direction and made evaluation comparatively direct. Unsupervised learning removes that answer key. The input is usually just a feature matrix $X$, and the goal is to discover or construct useful structure.
...
\begin{pitfall}
Do not evaluate an unsupervised method by inventing a convenient label after the fact and then treating that label as ground truth. Clusters may cut across known categories for legitimate reasons, and a visually pleasing two-dimensional plot can distort neighborhoods. State what property the method was designed to preserve.
\end{pitfall}
...
\begin{keyidea}
In supervised learning, a label can sometimes reveal that a poor representation is unhelpful. In unsupervised learning there may be no such corrective signal. The representation and distance choice therefore deserve explicit justification before interpreting the structure that appears.
\end{keyidea}
...
\begin{chapterrecap}
\begin{itemize}
  \item Unsupervised learning fits structure in $X$ without using a supplied label during fitting.
