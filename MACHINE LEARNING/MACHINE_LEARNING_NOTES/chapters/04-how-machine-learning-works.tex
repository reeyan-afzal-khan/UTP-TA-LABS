...
\begin{keyidea}
A machine-learning algorithm extracts useful structure from data. What counts as ``useful'' changes with the feedback signal. With labels, the structure should predict those labels. Without labels, the structure must be judged by stability, geometry, reconstruction, density, downstream usefulness, or domain meaning. With rewards, the structure must support good decisions over time.
\end{keyidea}
...
The details inside the boxes change. Supervised learning can compare predictions with known labels. Unsupervised learning often has no single correct answer and therefore needs several kinds of evidence. Reinforcement learning evaluates behavior across trajectories rather than independent labeled rows.
...
\midrule
What data are fitted? & Features $X$ plus label $y$ & Features $X$ only \\
...
\begin{description}[leftmargin=1.55in,style=nextline]
  ...
  \item[Stage 4A: unsupervised workflow] Reset the objective: no label is supplied during fitting, so evidence must come from geometry, stability, usefulness, or external meaning.
...
\begin{itemize}
  ...
  \item Supervised learning learns a label mapping; unsupervised learning learns structure without a supplied label; limited-label methods mix labeled and unlabeled evidence; reinforcement learning learns from rewards.
